---
title: "PEGASUS targeted sphingolipids"
author: "MHuang"
date: "8/18/2020"
output: 
  html_document: 
...
sph_dir <- "C:/Users/MHuang/Dropbox (Partners HealthCare)/PEGASUS/sphingolipids_targeted/data/"
dat_dir <- "data/"
fig_dir <- "figures/"
res_dir <- "results/sphingolipids/"


## Filenames

pheno_fname <- here(dat_dir, "pheno.RData")
sph_fname <- str_c(sph_dir, "eda_updated.RData")
...
## 1.2. Sphingolipids data 

```{r}

load(sph_fname, verbose = T)
setnames(mets_pheno, c("Group", "Original_Spl_ID"), c("SampleGroup", "sampleID_biobank"))
sph_dat <- merge(pheno, mets_pheno[, c(1:which(colnames(mets_pheno) == "LacCer22_0")), with = F], 
                 by = c("sampleID_biobank", "SampleGroup", "Match_ID", 
...
stratum_to_rm <- sph_dat[is.na(Collect_Date) | is.na(bmi) | abs(bmi_timegap_y) > 5, unique(Stratum)]
stratum_to_rm <- c(stratum_to_rm, sph_dat[, .N, Stratum][N == 1, Stratum]); length(stratum_to_rm)
sph_dat_trim <- sph_dat[!(Stratum %in% stratum_to_rm), ]

comorb_excl <- sph_dat_trim[copd_5 == 1 | a1ad == 1 | cf == 1 | bronchiectasis == 1 | cbronchitis == 1 | 
                                    immunodef == 1, unique(Stratum)]; length(comorb_excl)
sph_dat_excl <- sph_dat_trim[!(Stratum %in% comorb_excl), ]
...
feat_list <- sph_mets_info[pct_na_met <= missing_thld, feature]; length(feat_list)
...
dat_tmp <- sph_dat_trim[Stratum %in% sph_dat_trim[ocs_ard_serum_coll == 0, Stratum], ]
...
dat_tmp <- sph_dat_excl[Stratum %in% sph_dat_excl[ocs_ard_serum_coll == 0, Stratum], ]
...
dat_tmp <- sph_dat_excl[asthma_num == 1, ]
...
saveRDS(res_overall, file = here(res_dir, "res_overall_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Gender == "F", ]
...
dat_tmp2 <- sph_dat_excl[Gender == "F" & asthma_num == 1, ]
...
saveRDS(res_female, file = here(res_dir, "res_female_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Gender == 'M', ]
...
dat_tmp2 <- sph_dat_excl[Gender == "M" & asthma_num == 1, ]
...
saveRDS(res_male, file = here(res_dir, "res_male_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & ics_trim_totnum_1y > 0, Stratum], ]
...
dat_tmp2 <- sph_dat_excl[ics_trim_totnum_1y > 0 & asthma_num == 1, ]
...
saveRDS(res_ics_1y_y, file = here(res_dir, "res_ics_1y_y_sph.rds"))
...
saveRDS(res_ics_1y_n, file = here(res_dir, "res_ics_1y_n_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & eos >= 0.3, Stratum], ]
...
dat_tmp2 <- sph_dat_excl[asthma_num == 1 & eos >= 0.3, ]
...
saveRDS(res_eos_high, file = here(res_dir, "res_eos_high_sph.rds"))
...
saveRDS(res_eos_low, file = here(res_dir, "res_eos_low_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & 
...
dat_tmp2 <- sph_dat_excl[asthma_num == 1 & asthma_exacerbation_trim_totnum_1y > 0, ]
...
saveRDS(res_exac_1y_y, file = here(res_dir, "res_exac_1y_y_sph.rds"))
...
saveRDS(res_exac_1y_n, file = here(res_dir, "res_exac_1y_n_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & bmi >= 30, Stratum], ]
...
dat_tmp2 <- sph_dat_excl[asthma_num == 1 & bmi >= 30, ]
...
saveRDS(res_bmi_high, file = here(res_dir, "res_bmi_high_sph.rds"))
...
saveRDS(res_bmi_low, file = here(res_dir, "res_bmi_low_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & ocs_trim_totnum_1y > 0, Stratum], ]
...
dat_tmp2 <- sph_dat_excl[ocs_trim_totnum_1y > 0 & asthma_num == 1, ]
...
saveRDS(res_ocs_1y_y, file = here(res_dir, "res_ocs_1y_y_sph.rds"))
...
saveRDS(res_ocs_1y_n, file = here(res_dir, "res_ocs_1y_n_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & allergy_ever == 1, Stratum], ]
...
dat_tmp2 <- sph_dat_excl[asthma_num == 1 & allergy_ever == 1, ]
...
saveRDS(res_ige_high, file = here(res_dir, "res_allergy_y_sph.rds"))
...
dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & allergy_ever == 0, Stratum], ]
...
dat_tmp2 <- sph_dat_excl[asthma_num == 1 & allergy_ever == 0, ]
...
saveRDS(res_ige_norm, file = here(res_dir, "res_allergy_n_sph.rds"))
...
neut_median <- median(sph_dat_excl$neut, na.rm = T)
neut_median


## Asthmatics vs controls

dat_tmp1 <- sph_dat_excl[Stratum %in% sph_dat_excl[asthma_num == 1 & neut >= neut_median, Stratum], ]
...
dat_tmp2 <- sph_dat_excl[asthma_num == 1 & neut >= neut_median, ]
...
saveRDS(res_neut_high, file = here(res_dir, "res_neut_high_sph.rds"))
...
saveRDS(res_neut_low, file = here(res_dir, "res_neut_low_sph.rds"))
...
sph_dat_excl[asthma_num == 1, eos_high := case_when(eos >= 0.3 ~ 1, 
                                                          eos < 0.3 ~ 0)]
sph_dat_excl[asthma_num == 1, obese := case_when(bmi >= 30 ~ 1, 
                                                       bmi < 30 ~ 0)]
sph_dat_excl[, female := case_when(Gender == "F" ~ 1, 
                                       Gender == "M" ~ 0)]
sph_dat_asthma <- sph_dat_excl[asthma_num == 1, ]
...
sph_dat_ctrl <- sph_dat_excl[asthma_num == 0, ]
...
saveRDS(res_compare_asthmatics, file = here(res_dir, "res_compare_dif_asthmatics_sph.rds"))
...
sph_dat_asthma[, summary(eos)]
ggplot(sph_dat_asthma) + geom_histogram(aes(eos), bins = 100)
...
sph_dat_asthma[, summary(fev1_prebd_pctpred)]
ggplot(sph_dat_asthma) + geom_histogram(aes(fev1_prebd_pctpred), bins = 100)
...
sph_dat_asthma[, summary(fvc_prebd_pctpred)]
ggplot(sph_dat_asthma) + geom_histogram(aes(fvc_prebd_pctpred), bins = 100)
...
saveRDS(res_qt_asthmatics, file = here(res_dir, "res_qt_asthmatics_sph.rds"))
...
sph_dat_asthma[, summary(neut)]
ggplot(sph_dat_asthma) + geom_histogram(aes(neut), bins = 100)
