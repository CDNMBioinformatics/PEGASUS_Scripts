...
unmatched_stratum <- lipid_neg_dat[is.na(EMPI) | is.na(Collect_Date), unique(Stratum)]
lipid_neg_dat[, .N, .(ocs_ard_serum_coll, cortisone_ard_serum_coll, SampleGroup, asthma_num, Asthma_Status)]
asth_to_rm <- lipid_neg_dat[ocs_ard_serum_coll == 1 | is.na(bmi), Biobank_Subject_ID]; length(asth_to_rm)
lipid_neg_dat_trim <- lipid_neg_dat[!(Stratum %in% unmatched_stratum) & 
                                      !(Biobank_Subject_ID %in% asth_to_rm) & !(Match_ID %in% asth_to_rm), ]

lipid_neg_dat_trim[, .N, .(copd_5, a1ad, cf, bronchiectasis, cbronchitis, immunodef, asthma_num)][order(asthma_num)]
...
lipid_neg_dat_trim2 <- lipid_neg_dat_trim[!(Biobank_Subject_ID %in% comorb_excl) & !(Match_ID %in% comorb_excl), ]


## Subgroup by ICS use in 1 year before collection

lipid_neg_noics_1y <- lipid_neg_dat_trim[Stratum %in% lipid_neg_dat[SampleGroup == "Asthmatic" & ics_trim_totnum_1y == 0, Stratum], ]
lipid_neg_ics_1y <- lipid_neg_dat_trim[Stratum %in% lipid_neg_dat[SampleGroup == "Asthmatic" & ics_trim_totnum_1y >= 1, Stratum], ]


## Subgroup by OCS use in 1 year before collection

lipid_neg_noocs_1y <- lipid_neg_dat_trim[Stratum %in% lipid_neg_dat[SampleGroup == "Asthmatic" & ocs_trim_totnum_1y == 0, Stratum], ]
lipid_neg_ocs_1y <- lipid_neg_dat_trim[Stratum %in% lipid_neg_dat[SampleGroup == "Asthmatic" & ocs_trim_totnum_1y >= 1, Stratum], ]
...
res_lipid_neg <- copy(res_3_2)
save(lipid_neg_dat, lipid_neg_dat_trim, lipid_neg_dat_trim2, lipid_neg_mets_info, res_lipid_neg, 
     file = here(res_dir, "res_lipid_neg_clogi.RData"))
