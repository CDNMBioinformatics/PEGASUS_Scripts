...
subtitle: "Lipidomics Positive - kNN Imputation"
author: "MHuang"
date: "9/14/2020"
output: 
  html_document: 
...
pkg <- c("tidyverse", "data.table", "here", "readxl", "matrixStats", "e1071", "GGally", "ggrepel", "VIM")
for (p in pkg) {
...
dat_dir <- "C:/Users/MHuang/Dropbox (Partners HealthCare)/PEGASUS/global_metabolomics_082020/2020-08_PEGASuS_metabolomics data_ver1.0/"
fig_dir <- "figures/lipid_pos/"
post_qc_dir <- "post_qc_data/lipid_pos/"


## Filenames

dat_fname <- str_c(dat_dir, "2020-08-21_EXP00020_PEGASuS_LIPIDOMICS_POS_MSDIAL_PeakArea_curated_QCCorred_ForHarvard.xlsx")
dat_sheetname <- "DATA"
...
samp_info_cols <- colnames(dat)[1:(which(colnames(dat) == "F0")-1)]
mets_cols <- colnames(dat)[which(colnames(dat) == "F0"):ncol(dat)]
...
dat[, .N, .(SampleType, Group, Nonsmoker)]

samp_dat <- copy(dat[Group %in% c("Asthmatic", "Control"), ])
samp_dat[, length(unique(sampleID_biobank))] == nrow(samp_dat)

qc_dat <- copy(dat[Group == "QC", ])
...
samp_mets_cols <- colnames(samp_dat)[which(colnames(samp_dat) == "F0"):ncol(samp_dat)]
...
samp_dat <- kNN(samp_dat, variable = samp_mets_cols, dist_var = samp_mets_cols, imp_var = F)
...
ggplot(mets_pre_stats) + 
        ...
        labs(title = "Pre-processing IQR (knn imputed)") +
        labs(x = "Rank", y = "IQR") + 
        theme_minimal() + 
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "iqr_preprocessing_knn.png"), width = 5, height = 5)
...
ggplot(mets_pre_stats) + 
        ...
        labs(title = "Pre-processing skewness (knn imputed)") +
        labs(x = "Rank", y = "Skewness") + 
        theme_minimal() + 
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "skewness_preprocessing_knn.png"), width = 5, height = 5)
...
pc_pre <- cbind(pc_pre, samp_dat[, Group]) %>% setnames(., "V2", "Group")
ggplot(pc_pre, aes(x = PC1, y = PC2)) + 
        geom_point(aes(color = Group), size = 2, alpha = 0.5) + 
        labs(title = "Pre-processing PC2 vs PC1 (knn imputed)") + 
        theme_minimal() + 
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "PC1_PC2_preprocessing_knn.png"))
...
ggplot(pc_post, aes(x = PC1, y = PC2)) + 
        ...
        labs(title = "Post-processing PC2 vs PC1 (knn imputed)") + 
        theme_minimal() + 
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "PC1_PC2_postprocessing_knn.png"))

ggpairs(pc_post[, 1:10], aes(alpha = 0.1)) + 
        labs(title = "Post-processing PC1-10 pairwise plots (knn imputed)") + 
        theme_minimal() + 
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "PC1_PC10_pairs_postprocessing_knn.png"), width = 15, height = 12)
...
ggplot(mets_post_stats) + 
        ...
        labs(title = "Post-processing IQR (knn imputed)") +
        labs(x = "Rank", y = "IQR") + 
        theme_minimal() + 
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "iqr_postprocessing_knn.png"), width = 5, height = 5)
...
ggplot(mets_post_stats) + 
        ...
        labs(title = "Post-processing skewness (knn imputed)") +
        labs(x = "Rank", y = "Skewness") + 
        theme_minimal() + 
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "skewness_postprocessing_knn.png"), width = 5, height = 5)
...
ggplot(samp_dat_final, aes(x = PC1, y = PC2)) + 
        geom_point(aes(color = Group), size = 2, alpha = 0.5) + 
        labs(title = "Post-processing PC2 vs PC1 by asthma status (knn imputed)") + 
        geom_label_repel(aes(label = ifelse(PC1 > 80, sampleID_biobank, "")),
                  box.padding   = 0.25, 
                  point.padding = 0.5,
        ...
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "PC1_PC2_by_asthma_knn.png"))

ggplot(samp_dat_final, aes(x = PC1, y = PC2)) + 
        geom_point(aes(color = Age), size = 2, alpha = 0.5) + 
        scale_color_continuous(high = "#132B43", low = "#56B1F7") + 
        labs(title = "Post-processing PC2 vs PC1 by age (knn imputed)") + 
        geom_label_repel(aes(label = ifelse(PC1 > 80, sampleID_biobank, "")),
                  box.padding   = 0.25, 
                  point.padding = 0.5,
                  segment.color = 'grey50') + 
        ...
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "PC1_PC2_by_age_knn.png"))

ggplot(samp_dat_final, aes(x = PC1, y = PC2)) + 
        geom_point(aes(color = Gender), size = 2, alpha = 0.5) + 
        labs(title = "Post-processing PC2 vs PC1 by gender (knn imputed)") + 
        geom_label_repel(aes(label = ifelse(PC1 > 80, sampleID_biobank, "")),
                  box.padding   = 0.25, 
                  point.padding = 0.5,
                  segment.color = 'grey50') + 
        ...
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "PC1_PC2_by_gender_knn.png"))

ggplot(samp_dat_final, aes(x = PC1, y = PC2)) + 
        geom_point(aes(color = Race), size = 2, alpha = 0.5) + 
        labs(title = "Post-processing PC2 vs PC1 by race (knn imputed)") + 
        geom_label_repel(aes(label = ifelse(PC1 > 80, sampleID_biobank, "")),
                  box.padding   = 0.25, 
                  point.padding = 0.5,
                  segment.color = 'grey50') + 
        ...
        theme(plot.title = element_text(size = 12, face = "bold"), 
              axis.text = element_text(size = 12), 
              axis.title = element_text(size = 12))
ggsave(here(fig_dir, "PC1_PC2_by_race_knn.png"))
...
save(samp_dat_final, samp_mets_cols, mets_info, samp_na, samp_dat_origscale, 
     file = here(post_qc_dir, "processed_lipid_pos_knn.RData"))
