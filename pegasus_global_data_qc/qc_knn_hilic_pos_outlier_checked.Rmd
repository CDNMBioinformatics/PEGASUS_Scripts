...
subtitle: "HILIC Positive - kNN Imputation"
author: "MHuang"
date: "11/2/2020"
output: 
  html_document: 
...
fig_dir <- "figures/hilic_pos/outlier_checked/"
post_qc_dir <- "post_qc_data/hilic_pos/outlier_checked/"


## Filenames

dat_fname <- str_c(dat_dir, "2020-09-25_EXP00020_PEGASuS_HILIC_POS_RTCorr_MSDIAL_curated_PeakArea_QCCorred_ForHarvard_outlier checked.xlsx")
dat_sheetname <- "Data"
...
samp_info_cols <- colnames(dat)[1:(which(colnames(dat) == "F113")-1)]
mets_cols <- colnames(dat)[which(colnames(dat) == "F113"):ncol(dat)]

mets_info <- read_xlsx(dat_fname, sheet = feat_sheetname) %>% as.data.table()
setnames(mets_info, c("feature", "label", "rt_min", "mz", "tentative_annotation"))
...
samp_mets_cols <- colnames(samp_dat)[which(colnames(samp_dat) == "F113"):ncol(samp_dat)]
...
ggplot(samp_dat_final, aes(x = PC1, y = PC2)) + 
        ...
        geom_label_repel(aes(label = ifelse(sampleID_biobank == "S-001693274", sampleID_biobank, "")),
                  box.padding   = 0.25, 
                  point.padding = 0.5,
                  segment.color = 'grey50') + 
        theme_minimal() + 
...
save(samp_dat_final, samp_mets_cols, mets_info, samp_na, samp_dat_origscale, 
     file = here(post_qc_dir, "processed_hilic_pos_knn.RData"))
